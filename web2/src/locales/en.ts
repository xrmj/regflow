...
Example: A 1 KB message with 1024-dim embedding uses ~9 KB. The 5 MB default limit holds ~500 such messages.`,
    ...
    knowledgeDetails: {
      ...
      retrievalTestingDescription:
        'Conduct a retrieval test to check if LocalSpace can recover the intended content for the LLM.',
      Parse: 'Parse',
      dataset: 'Dataset',
      ...
      testingDescription:
        'Conduct a retrieval test to check if LocalSpace can recover the intended content for the LLM. If you have adjusted the default settings, such as keyword similarity weight or similarity threshold, to achieve the optimal results, be aware that these changes will not be automatically saved. You must apply them to your chat assistant settings or the Retrieval agent component settings.',
      similarityThreshold: 'Similarity threshold',
      ...
      rerankTip: `Optional. If left empty, LocalSpace will use a combination of weighted keyword similarity and weighted vector cosine similarity; if a rerank model is selected, a weighted reranking score will replace the weighted vector cosine similarity. Please be aware that using a rerank model will significantly increase the system's response time. If you wish to use a rerank model, ensure you use a SaaS reranker; if you prefer a locally deployed rerank model, ensure you start LocalSpace with docker-compose-gpu.yml.`,
      topK: 'Top-K',
...
`,
    ...
    knowledgeConfiguration: {
      ...
      tableColumnModeAutoDescription:
        'All columns are included in chunk text and stored as metadata (LocalSpace default).',
...
**Essential Rules:**
  ...
  - **Always consider** the entire conversation history.`,
      ...
      **Essential Rules:**
      ...
      betaError:
        'Please acquire a LocalSpace API Key from the System Settings page first.',
      searching: 'Searching...',
      parsing: 'Parsing',
      uploading: 'Uploading',
...
Example: Virtual Hosted Style`,
      ...
      embeddingModelTip:
        'The default embedding model for each newly created dataset. If you cannot find an embedding model from the dropdown, check if you are using LocalSpace slim edition (which does not include embedding models) or check https://ragflow.io/docs/dev/supported_models to see if your model provider supports this model.',
      img2txtModel: 'VLM',
      img2txtModelTip:
